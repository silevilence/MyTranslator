...
- [x] **后端：术语表管理 API 与术语对齐检查**（术语 CRUD + 对齐校验接口）
  - [x] 术语条目增删改查与模糊匹配查询接口（供翻译/规则引擎复用）
  - [x] 术语对齐检查接口：校验译文是否使用规定术语，输出未对齐分段
  - [x] 更新 `docs/back` 术语接口约定
  - 验收：术语 CRUD 生效；包含规定术语的译文通过检查，未使用术语的译文被检出并指出期望术语

- [x] **前端：术语表管理界面**（术语 CRUD + 搜索）
  - [x] 术语列表、搜索、新增/编辑/删除操作界面
...
- [x] **前端：AI 审核意见展示**（触发审核 + 按分段展示意见）
  - [x] 审核触发：任务详情页操作区"审核"按钮（紧邻翻译按钮），复用提供商/模型选择器与配置异常提示；运行中按钮 loading 防重复提交、翻译按钮禁用（同锁互斥）；进度复用长任务模式（进度条 + 百分比 + 阶段文案），Retry-After 轮询；失败按 `code` 映射文案并可重试；页面刷新后取最新审核运行恢复进度
  - [x] 分段列表问题标记：有意见的分段行显示 info 色三通道徽标（图标 + "N 条意见"），与规则违规（error）徽标可共存
  - [x] `ReviewPanel` 审核意见区块：分段行内展开，意见按严重度降序展示，每条 = 严重度徽标 + 问题描述 + 修改建议（建议为空显示"无建议"）；只读查看，不提供一键应用（`SegmentEditorPanel` 详情面板未实现，落点由「翻译编辑器基础」承接）
  - [x] 审核运行期间逐批提交的意见随分段刷新出现；运行完成/部分失败有明确提示；无意见分段不显示标记，详情面板空态"暂无审核意见"
  - [x] 验收：执行审核后分段列表出现意见徽标，分段内按严重度展示问题描述与修改建议；无意见分段不显示标记；审核失败/互斥有明确提示；刷新后意见与审核运行状态一致

- [x] **后端：翻译记忆库(TM)与历史对比、差异告警**（TM 存储匹配 + 对比参考）
  - [x] TM 存储：历史译文入库与相似度模糊匹配接口
  - [x] 历史翻译对比接口：返回相似历史译文供参考
  - [x] TM 差异告警：译文与相似历史译文差异过大时标记告警
  - [x] 更新 `docs/back` TM 接口约定
  - 验收：录入历史译文后可被匹配到（含相似度）；差异过大的译文被标记告警；按约定文档调用对比接口返回历史译文

- [x] **前端：历史翻译对比与差异告警界面**（相似译文参考 + 对比视图）
  - [x] 相似历史译文展示、差异对比视图、告警提示
  - 验收：匹配到历史译文时展示对比内容，差异过大可见告警标记

- [x] **后端：硬性规则检查引擎**（可扩展确定性规则 + 内置占位符/漏翻检查）
  - [x] 规则引擎框架：规则独立于核心流程，可配置、可新增而无需改动核心代码
  - [x] 内置占位符保留检查：校验特殊标记/代码符号未被破坏，含成对性校验（`<xN>`/`</xN>` 配对、顺序、自闭合）
  - [x] 内置漏翻检测：发现未翻译/缺失的分段
  - [x] 检查结果接口：违规类型、定位分段/位置
  - [x] 更新 `docs/back` 规则检查接口约定
  - 验收：破坏占位符的译文被检出、空译文分段被检出；新增一条规则不改动核心流程即可生效

- [x] **后端：任务管理与开放 REST 接口**（任务生命周期 + 对外 API）
  - [x] 任务生命周期管理：创建/处理中/完成/失败，进度统计（分段完成数/总数；受保护块不计入分段）
  - [x] 分段数据模型为公共契约：分段文本（含 `<xN>` 引用）+ 标记表 + 受保护块 + 章归属（epub）；译文保存只存可译文本，标记表不可变
  - [x] 译文人工编辑保存 API（供编辑器调用，保存分段译文与确认状态）
  - [x] 对外开放：第三方系统通过同一套 API 创建任务、查询状态、获取译文结果，无内外部接口区分
  - [x] 更新 `docs/back` 任务接口约定
  - 验收：任务状态按流程流转且进度正确；第三方按文档独立完成「创建任务→翻译→编辑→获取结果」全链路；`docs/back` 文档可直接指导外部对接

- [x] **前端：翻译编辑器基础**（分段编辑、保存、确认状态）
  - [x] 分段列表、译文编辑与保存（按 `docs/back` 译文保存约定）、翻译确认状态管理；占位符 `<xN>` 以可读样式渲染、hover 查看标记含义，译文编辑不可删除/改写占位符（违规由规则引擎标记）
  - [x] 分段详情面板（`SegmentEditorPanel`，交互规范 §2 双栏右侧）：将审核意见区块从分段行内展开迁入详情面板（组件规范 §3 `ReviewPanel` 契约）
  - [x] 批量操作与基础快捷键
  - 验收：编辑保存后刷新数据不丢失；确认状态可追踪并反映到任务进度

- [x] **前端：翻译编辑器增强**（集成规则检查、TM 参考、术语提示）
  - [x] 编辑器内展示规则违规标记（占位符破坏/漏翻）、TM 相似历史译文参考、术语对齐提示
  - 验收：编辑器内违规分段可见标记，有参考译文与术语提示的分段可见对应信息

- [x] **前端：任务管理与进度看板、Token 管理页**（任务列表 + 看板 + 后台管理）
  - [x] 任务列表/创建入口、状态筛选、进度看板（完成度百分比展示）
  - [x] API Token 管理页：生成、撤销、列表展示（配合后端 Token 管理 API）
...
- [x] **后端：Docker 容器化**（后端镜像 + compose 编排）
  ...
  - [x] `INITIAL_TOKEN` 通过环境变量注入容器
  - [x] AI 提供商/模型配置经配置管理 API 写入（ADR-0003：密钥存 SQLite，不再经环境变量注入；提供部署后 API 配置与触发翻译示例）
